...
<head>
	...
	<title>e-mporium: Maru Maru Pottery</title>
...
<?php
	...
	$query = "select p.product_name,p.product_desc,p.product_url, p.product_id, ifnull(avg(r.rating),0), count(r.rating), count(r.review) from  products p left join rating r on r.product_id = p.product_id where vendor_id='maru' group by p.product_id order by p.product_id";
	 if ( !( $database = mysql_connect( "localhost","vishwasm_root", "root123" ) ) ) {                          
            ...
            $reviewsQuery = "select r.review, r.product_id, r.review_date, r.user_id, IFNULL( r.rating, 0 ) as rating from rating r join products pr on pr.product_id = r.product_id where pr.vendor_id='maru' and r.review is not NULL and LENGTH(r.review)>0 order by r.product_id, r.review_date";		
...
?> 
	...
	<?php for($i=0;$i<10;$i++){
		...
		echo '<figure><img src="../img/prod/page6_img'.($i+1).'.jpg"></figure>';
		echo '</td>';
		echo '<td>';
	...
	<div id="bg">
		<img src="../img/bg/bg6.jpg" alt="error">
	</div>
